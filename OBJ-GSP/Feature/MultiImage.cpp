...
MultiImages::MultiImages(const string& _file_name,
	...
	const vector<cv::Point2f>& _custom_pts1,
...
}// <--- 这里是构造函数的终点，确保后面没有多余的 } 或 emplace_back
...
void MultiImages::doFeatureMatching() const {
	...
	for (int i = 0; i < images_match_graph_pair_list.size(); ++i) {
		...
		pairwise_matches[pm_index].confidence = 2.; 
		...
		pairwise_matches[pm_index].H = apap_homographies[m1][m2].front(); 
	}
}
...
const vector<vector<vector<Point2> > >& MultiImages::getFeatureMatches() const {
	if (feature_matches.empty()) {
		int n = images_data.size();
		feature_matches.resize(n);
		for (int i = 0; i < n; ++i) {
			feature_matches[i].resize(n);
		}

		if (!custom_pts1.empty() && !custom_pts2.empty()) {
			cout << ">>> 💡 [原版核心唤醒] 准备挂载 " << custom_pts1.size() << " 对原始特征点。" << endl;
			
			// 1. 极高精度筛选：将 RANSAC 阈值从 5.0 降低到 1.5，只留下最稳、最准的点
			vector<uchar> mask;
			Mat H = cv::findHomography(custom_pts1, custom_pts2, cv::RANSAC, 1.5, mask);
			
			// 2. 抽稀策略：网格不需要几千个点，几百个足够。
			// 我们每隔 10 个点取一个，这样可以覆盖全图，又不会让约束过载
			int step = 10; 
			int inlier_count = 0;
			int injected_count = 0;

			feature_matches[0][1].reserve(500); 
			feature_matches[1][0].reserve(500);

			for (size_t k = 0; k < custom_pts1.size(); ++k) {
				if (mask[k]) {
					inlier_count++;
					// 只有 Inlier 且满足步长要求的点才被注入
					if (inlier_count % step == 0) {
						feature_matches[0][1].emplace_back(custom_pts1[k].x, custom_pts1[k].y);
						feature_matches[1][0].emplace_back(custom_pts2[k].x, custom_pts2[k].y);
						injected_count++;
					}
				}
			}
			cout << ">>> 🛡️ [精英提纯] 总计 " << inlier_count << " 个 Inliers。" << endl;
			cout << ">>> 🛡️ [抽稀完毕] 最终注入 " << injected_count << " 对高质量点位，网格现在能“呼吸”了！" << endl;
...
{   /// Interpolate the sampling points with the four vertices of the mesh
	if (content_mesh_interpolation.empty()) {
		content_mesh_interpolation.resize(images_data.size());
		const vector<vector<vector<Point>>>& content_sample_points = getContentSamplePoints(); //<a set of image < an image < a contour < point>>>>
