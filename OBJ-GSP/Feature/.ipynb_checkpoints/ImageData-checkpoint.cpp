...
DEFINE_string(pre_model, "sam_ckpt/sam_preprocess.onnx", "Path to the preprocessing model");
DEFINE_string(sam_model, "sam_ckpt/sam_vit_h_4b8939.onnx", "Path to the sam model");
DEFINE_string(image, "sam_ckpt/test_input.jpg", "Path to the image to segment");
...
ImageData::ImageData(const string& _file_dir,
	...
	// ==========================================================
	// 【终极解除封印】：严禁程序对图片进行任何形式的缩放！
	// 把下面这几行彻底注释掉，让 1700+ 的高清原图直接进入内存！
	// ==========================================================
...
const vector<vector<Point>> ImageData::getContentSamplesPoint(vector<double>& weights) const
{
	...
	if (python_sam)
	{
		...
		while (std::getline(file, line)) {
			if (line.find("Ann") != std::string::npos) {
				if (!current_contour.empty()) {
					if (current_contour.size() < 1) {
				...
				char comma;
				if (iss >> x >> comma >> y) {
					if ((x >= margin) && (x < img.cols - margin) &&
						(y >= margin) && (y < img.rows - margin)) {
						current_contour.push_back({ x, y });
					}
					else
					{
						use_contour = false; 
	...
	for (vector<vector<Point>>::iterator iterator = res.begin(); iterator != res.end() && i < lineslength.size(); ++iterator, ++i) {
		...
		sort((*iterator).begin(), (*iterator).end(), sortForPoint); // sort with increasing x coordinates
		...
		cv::Mat imgDrawCounters;
		img.copyTo(imgDrawCounters);
		cv::Scalar pointColor0(0, 0, 255); // red
		for (int j = 0; j < (*iterator).size(); j++) {
			circle(imgDrawCounters, (*iterator)[j], 3, pointColor0, FILLED);
		}
#endif
		index++;
	}
	...
	if (true) {
		...
		for (auto it = samplesData.begin(); it != samplesData.end(); ) {
			bool removePoints = false;

			for (auto pointIt = it->begin(); pointIt != it->end(); ) {
				const cv::Point& point = *pointIt;
				if (point.x < margin || point.x >= img.cols - margin ||
					point.y < margin || point.y >= img.rows - margin) {
					pointIt = it->erase(pointIt);
					removePoints = true;
				}
				else {
					++pointIt;
	...
	std::vector<std::vector<Point>> samplesDataOut;
	...
	if (python_sam == false) {
		samplesDataOut = samplesData;
	}
	else {
		cv::Scalar pointColor2(0, 255, 0); //green

		int threshold = 5;
		for (const auto& sampledata : samplesData) {
			// cout << sampledata.size() << endl;
			if (sampledata.size() < 10) { continue; }
			...
			mean_point.x /= sampledata.size();
			mean_point.y /= sampledata.size();
			for (size_t i = 1; i < sampledata.size(); ++i) {
				if (sampledata[i].x < threshold || sampledata[i].y < threshold
					|| sampledata[i].x >= (img.cols - threshold) || sampledata[i].y >= (img.rows - threshold)) {
					continue;
				}
				std::vector<Point> vector_points, vector_points2;
				vector_points.push_back(mean_point);
				vector_points.push_back(sampledata[i]);
				vector_points.push_back(sampledata[i - 1]);
				samplesDataOut.push_back(vector_points);

				vector_points2.push_back(mean_point);
				vector_points2.push_back(sampledata[i - 1]);
				vector_points2.push_back(sampledata[i]);
				samplesDataOut.push_back(vector_points2);
			}

		}
	}	
	return samplesDataOut;
}
